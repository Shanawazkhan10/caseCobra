...
const Page = () => {
    ...
    const [updateProgress, setUpdateProgress] = useState<number>(45)
    ...
    const onDropRejected = (rejectedFiles: FileRejection[]) => {
        ...
        toast({
            ...
            description: "Choose a image files",
            variant: "destructive"
        })
                        ...
                        <section>
                            <div className='h-full w-full flex-1 flex flex-col items-center justify-center' {...getRootProps()}>
                                ...
                                {isDragOver ? <MousePointerSquareDashed className='h-6 w-6 text-zinc-500 mb-2' /> :
                                    (isUpload || isPending) ?
                                        <Loader2 className='animated-spin h-6 w-6 text-zinc-500 mb-2' /> : <Image className='h-6 w-6 text-zinc-500 mb-2' />}
                                <div className='flex flex-col justify-center mb-2 text-sm text-zinc-700'>
                                    {isUpload ? <div className='flex flex-col items-center'>
                                        <p>Uploading...</p>
                                        <Progress value={updateProgress} className='mt-2 w-40 h-2 bg-gray-300' /></div>
                                        : isPending ? <div className='flex flex-col items-center'>
                                            <p>Redorection, Please wait...</p>
                                        </div>
                                            : isDragOver ? <p><span className='font-semibold'>Drop file</span>to upload</p>
